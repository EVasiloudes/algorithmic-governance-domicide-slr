#!/usr/bin/env python3
"""
pipeline_01_extract.py v2 — Local RIS export → PDF → text → metadata CSV

Pipeline step 1/3. Reads from the Zotero RIS export + files directory.
Uses the Zotero SQLite database to map RIS records to their PDF files
(by matching on title).

Usage:
    python3 pipeline_01_extract_v2.py
"""
...
EXPORT_BASE = REPO / "Diss_Methods_Data" / "{exported from Zotero}"
export_dirs = [d for d in EXPORT_BASE.iterdir() if d.is_dir() and "SLR-INCLUDED-RECORDS" in d.name]
if not export_dirs:
    raise FileNotFoundError(f"Could not find SLR-INCLUDED-RECORDS export dir in {EXPORT_BASE}")
...
def build_pdf_lookup():
    """Build a lookup from normalised title → (itemID, pdf_filename) using Zotero DB.

    Returns a dict: normalised_title -> (itemID, pdf_filename)
    Also returns a set of (itemID, pdf_filename) pairs for the backup collection.
    """
    db_tmp = Path("/tmp/zotero_pipeline_lookup.sqlite")
    shutil.copy2(str(ZOTERO_DB), str(db_tmp))
    conn = sqlite3.connect(str(db_tmp))
    c = conn.execute("""
        SELECT i.itemID, idv.value, ia.path
        FROM items i
        JOIN collectionItems cmi ON i.itemID = cmi.itemID AND cmi.collectionID = 94
    ...
    lookup = {}  # normalised_title -> [(itemID, pdf_filename)]
    for item_id, title, path_str in rows:
        norm = _normalise(title)
        # path is "storage:FILENAME.pdf"
...
def _normalise(s):
    """Normalise a string for matching: lowercase, strip, collapse whitespace."""
    s = s.lower().strip()
    s = re.sub(r'\s+', ' ', s)
...
def find_pdf_for_record(rec, files_dir, pdf_lookup):
    """Find the PDF for a RIS record in the export files/ subdirs.

    Uses the Zotero DB lookup table for exact title matching, then falls back
    to fuzzy author/year/title matching.
    """
    ...
    if norm_title in pdf_lookup:
        candidates = pdf_lookup[norm_title]
        for item_id, pdf_filename in candidates:
            subdir = files_dir / str(item_id)
            pdf_path = subdir / pdf_filename
            if pdf_path.exists():
                return pdf_path
            # Try finding any PDF in the subdir
            if subdir.is_dir():
                pdfs = list(subdir.glob("*.pdf"))
                if pdfs:
                    return pdfs[0]

    # 2. Fuzzy match: author surname + year + title words
    ...
    for subdir in files_dir.iterdir():
        ...
        for f in subdir.iterdir():
            ...
            # Author surname match (strongest signal)
...
def extract_text_from_pdf(pdf_path):
    ...
    try:
        ...
        if len(full_text.strip()) < 50:
            raise ValueError("Too little text extracted, trying fallback")
        return full_text
    except Exception:
        try:
...
def main():
    log("=" * 60)
    log("SLR Pipeline 01 v2: Local RIS Export → PDF → Text Extraction")
    ...
    # ── Build PDF lookup from Zotero DB ────────────────────────────────
    log("Building PDF lookup from Zotero database...")
    ...
    log_data = {
        ...
        "pipeline": "pipeline_01_extract_v2",
